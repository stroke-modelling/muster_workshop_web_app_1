        ...
        if treat_type in ['ivt', 'mt']:
            ...
            for key, df in mrs_lists_dict.items():
                ...
                for c, col_here in enumerate(cols_time):
                    ...
                    df = df.rename(columns=dict(zip(dist_cols, dist_cols_suff)))
            ...
            if len(multi_scens) == 0:
                ...
                for s in multi_scens:
                    ...
                    cols_group += [f'{s}_time_to_mt', f'{s}_time_to_ivt', f'{s}_lvo_ivt_better_than_mt']
            ...
            for key, df in mrs_lists_dict.items():
                ...
                for c, (col_ivt, col_mt) in enumerate(cols_time):
                    ...
                    if occ_type == 'lvo':
                        ...
                        # Rename "mt" to "ivt_mt":
                        # dist_cols_here_suff = [d+suffixes[c] for d in dist_cols_here]
                        df = df.rename(columns=dict(zip(dist_mt_cols, dist_cols_here)))
                        # df = df.rename(columns=dict(zip(dist_mt_cols, dist_cols_here)))
                        # Now copy over all IVT results:
                        df = pd.merge(
                            df, df_mrs_ivt[['time_to_ivt'] + dist_ivt_cols],
                            left_on=col_ivt, right_on='time_to_ivt', how='left',
                            # suffixes=(suffixes[c], None),
                        )
                        # dist_cols_suff = [d+suffixes[c] for d in dist_ivt_cols]
                        # df = df.rename(columns=dict(zip(dist_ivt_cols, dist_cols_suff)))
                        ...
                        # dist_cols_here_suff = [d+suffixes[c] for d in dist_cols_here]
                        df = df.rename(columns=dict(zip(dist_ivt_cols, dist_cols_here)))
                        # df = df.rename(columns=dict(zip(dist_ivt_cols, dist_cols_here)))
...
def find_total_mrs_for_unique_times_multiscen(
        dict_of_dfs,
        lsoa_names,
        treat_type,
        occ_types,
    ...
    for occ_type in occ_types:
        if treat_type in ['ivt', 'mt']:
            ...
            for key, df in mrs_lists_dict.items():
                ...
                try:
                    dict_dfs_to_store[key] = pd.merge(
                        dict_dfs_to_store[key], df[dist_cols],
                        left_index=True, right_index=True
                        )
                except KeyError:
                    dict_dfs_to_store[key] = df
